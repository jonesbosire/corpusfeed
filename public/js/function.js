(function ($) {
	...
	if ($cf) {
		$cf.addEventListener('submit', function (e) {
			...
			var existing = $cf.querySelector('.cf-msg');
			if (existing) existing.remove();

			var token = $cf.querySelector('input[name="_token"]');
			...
			.then(function(data){
				...
				el.className = 'cf-msg';
				if (data.success) {
					el.style.cssText = 'margin-top:16px;padding:14px 18px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;color:#166534;font-size:15px;';
					el.textContent = data.message;
					$cf.reset();
					if (span) span.textContent = 'Message Sent!'; else btn.textContent = 'Message Sent!';
					setTimeout(function(){
						btn.disabled = false;
						if (span) span.textContent = original; else btn.textContent = original;
					}, 4000);
				} else {
					el.style.cssText = 'margin-top:16px;padding:14px 18px;background:#fef2f2;border:1px solid #fecaca;border-radius:8px;color:#991b1b;font-size:15px;';
					el.textContent = data.message || 'Something went wrong. Please try again.';
					btn.disabled = false;
					if (span) span.textContent = original; else btn.textContent = original;
				}
				$cf.appendChild(el);
